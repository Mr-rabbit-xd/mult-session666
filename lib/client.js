// lib/index.js (updated to use per-session group cache)
...
async function onConnected(sessionId) {
  try {
    ...
    // handle group participant updates and dispatch to plugins (session-scoped cache)
    sock.ev.on("group-participants.update", async (event) => {
      try {
        ...
        // register groups.update once per connected socket (place after anticall handler)
        sock.ev.on("groups.update", async (events) => {
          for (const ev of events || []) {
            try {
              ...
              try {
                // try to refresh full metadata from server (best-effort)
                const md = await sock.groupMetadata(jid);
                if (md) sessionCache.setCached(jid, md);
              } catch (err) {
                console.warn(`[${sessionId}] Failed to fetch metadata for ${jid}:`, err?.message ?? err);
              }
            } catch (err) {
    ...
    // messages.upsert handler (unchanged behavior, kept for context)
      ...
      try {
        ...
        try {
          if (entry?.serializer && typeof entry.serializer.serializeSync === "function") {
            ...
            // no serializer instance available — fallback to raw message
            msg = raw;
          }
        } catch (e) {
